...
//bearing helper: 0 deg = north, cw positive
...
//meters so tiny jitter doesnt count as movement ~0.75 m
...
//if yaw provided, accept rad or deg
...
//build DivIcon whose inner IMG we rotate ourselves
...
let lastAppliedMs = 0;//newest timestamp weve rendered
...
function refreshDrone() {
  ...
  //abort any prev request(prevents out of order paints)
...
function updateMarker(pkt, hdg) {
  ...
  if (!droneMarker) {
    ...
    //rotate the inner img explicitly; nothing can overwrite it, its been a problem
  ...
  //keep map following the drone w/out animation jitter
